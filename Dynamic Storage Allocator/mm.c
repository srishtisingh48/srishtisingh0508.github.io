...
void* mm_malloc(size_t size) {
 ...
 // You will want to replace this return statement...
  //getting a block that is large enough
  ptr_free_block = search_free_list(req_size);

  //requesting more space if we run out of space in the heap
 ...
 //removes the free block from the free list 
  ...
  //updates boundary tags
  ptr_free_block->size_and_tags += TAG_USED;
  block_size = SIZE(ptr_free_block->size_and_tags);

  //splits this block,if possible, and inserts the remaining back into the free list
  if(block_size-req_size>=MIN_BLOCK_SIZE){
	  //updates boundary tags
	  ptr_free_block->size_and_tags = req_size + TAG_USED + TAG_PRECEDING_USED;

	  block_info* split_block_ptr = (block_info*) UNSCALED_POINTER_ADD(ptr_free_block, req_size);
          split_block_ptr->size_and_tags = (block_size - req_size);
	  split_block_ptr->size_and_tags += TAG_PRECEDING_USED;

	  block_info* footer_block = UNSCALED_POINTER_ADD(split_block_ptr, (block_size-req_size)-WORD_SIZE);
	  footer_block->size_and_tags = (block_size - req_size) + TAG_PRECEDING_USED;

	  insert_free_block(split_block_ptr);

  } else{
	  block_info* next_block_ptr = UNSCALED_POINTER_ADD(ptr_free_block, block_size);
	  next_block_ptr->size_and_tags |= TAG_PRECEDING_USED;
  }
...
void mm_free(void* ptr) {
  ...
  //getting pointer to the payload of the block to free
  block_to_free = (block_info*)UNSCALED_POINTER_SUB(ptr, WORD_SIZE);
  payload_size = SIZE(block_to_free->size_and_tags) - WORD_SIZE;
  
  //updating header and footer size_and_tags to change the used tag
  size_t preceding_block_tag = block_to_free->size_and_tags & TAG_PRECEDING_USED;
  block_to_free->size_and_tags = SIZE(block_to_free->size_and_tags) | preceding_block_tag;
  ((block_info*)UNSCALED_POINTER_ADD(block_to_free,payload_size))->size_and_tags = block_to_free->size_and_tags;
  
  //inserting the free block back into the free list and coalescing the following blocks
